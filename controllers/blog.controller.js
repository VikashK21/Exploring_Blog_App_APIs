...
class blogCtrl {
    ...
    edit_blog = async (req, res) => {
        ...
        try {
            const result = await Blog_T.edit_blog(req.body);
    ...
    likes_dislikes = async (req, res) => {
        ...
        //     dislikes: joi.boolean().default(false)
        // })
        // const schemaValidated = schemaValidate.validate(req.body);
        // if (schemaValidated.error) {
        //     return res.status(415).json(schemaValidated.error.details)
        ...
        try {
            ...
            const result = await Blog_T.likes_dislikes(Number(req.params.id), req.user_id, req.body);
            res.json({result});
        } catch (err) {
            res.status(400).json(err.message);
        }
    ...
    del_blog = async (req, res) => {
        try {
            const result = await Blog_T.del_blog(Number(req.params.id))
            res.json({result})
        } catch (err) {
            res.status(400).json(err.message);
